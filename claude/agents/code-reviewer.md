...
1. **Scope the change.** Read the diff first (working tree, staged, named commits, or a branch versus its merge target). Understand what the change claims to do before judging it.
...
1. Read the diff. For a branch, compare it to its merge target using the common ancestor: `git merge-base <base> <branch>` then review `<ancestor>..<branch>`. Resolve the base as the repository default branch (`git symbolic-ref refs/remotes/origin/HEAD`, else `main`, else `master`).
