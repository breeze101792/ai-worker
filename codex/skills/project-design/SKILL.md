...
Codex has no `plan` primary, so the main agent adopts the Plan-department
posture: read-only planning, no source edits. It authors and assembles the
docs. It consults the custom agents below for the deep sections, then writes
