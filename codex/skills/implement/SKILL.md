---
name: implement
description: Implement a feature or fix by dispatching to the right domain engineer — firmware, Python, or web — based on the task. Invoke when code must be written or changed and the correct specialist is not already selected.
---

Implement the requested change by delegating to the domain engineer that fits. The task comes from the user's prompt — if empty, ask what to implement and where.
...
- Never implement the change yourself in this skill — dispatch to the specialist and let it work.
